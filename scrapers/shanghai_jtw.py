...
class ShanghaiJTWScraper(BaseScraper):
    """上海市交通委员会网站爬虫"""
    ...
    def search_with_selenium(self, keyword, date_filter=None, section_filter='all'):
        results = []
        search_url = f"{self.base_url}/websearch/index.html#search/query={quote(keyword)}"
    ...
    def _scrape_all_pages(self, driver):
        ...
        while current_page <= max_pages:
            ...
            if not result_items:
                result_items = soup.select('div.result-item') or soup.select('div[class*="result"]')
    ...
    def _parse_result_item(self, item, seen_urls):
        ...
        if 'websearch' in full_url or full_url in seen_urls:
            return None
