...
"""
上海市科学技术委员会网站爬虫
网站: https://stcsm.sh.gov.cn
搜索页面: https://stcsm.sh.gov.cn/searchAll/index.html#search/query=关键词
"""
...
class ShanghaiSTCSMScraper(BaseScraper):
    """上海市科学技术委员会网站爬虫"""
    ...
    def search_with_selenium(self, keyword, date_filter=None, section_filter='all'):
        results = []
        search_url = f"{self.base_url}/searchAll/index.html#search/query={quote(keyword)}"
        ...
        try:
            if not self.safe_get_page(driver, search_url, max_retries=3, wait_after_load=5):
                self.log("[错误] 搜索页面加载失败", "error")
                return results

            self.log("页面加载完成，等待搜索结果...")
            time.sleep(3)
            ...
            if section_filter and section_filter != 'all':
                ...
                if not clicked:
                    self.log(f"[警告] 板块「{section_name}」无法访问或不存在，将使用全部板块搜索")
    ...
    def _click_section_filter(self, driver, section_filter):
        ...
        try:
            ...
            self.log(f"[警告] 板块切换可能未成功", "warning")
            return False
    ...
    def _scrape_all_pages(self, driver):
        ...
        self.log(f"共爬取 {current_page} 页，提取到 {len(results)} 条有效结果")
        return results
    ...
    def scrape(self, keywords, start_date, end_date=None, date_filter=None, section_filter='all', fetch_content=True, **kwargs):
        results = []
        seen_urls = set()
        ...
        try:
            ...
            if results and fetch_content:
                print(f"\n开始提取正文内容（已筛选 {len(results)} 条含书名号的政策）...")
                ...
                for idx, result in enumerate(results, 1):
                    ...
                    if url:
                        ...
                        content_result = self.extract_article_content(
                            url,
                            content_selectors=['#ivs_content', '.xxgk_content_nr', 'div.xxgk_content_nr'],
